...
public class GameService {
	...
	public boolean joinGame(String userId, String username, String gameId) {
		if (!players.containsKey(userId)) { //Si Le joueur est déjà dans une autre partie
	...
	public boolean createBotGame(String userId, String username, String gameId, String level) {
		if (!players.containsKey(userId)) { // Si Le joueur est déjà dans une autre partie
			players.put(userId, new Player(username));
			System.out.println("Création de la partie n°" + gameId);
			try {
				System.out.println("ici");
				games.put(gameId, new GameFactory().createGameWithAI(players.get(userId), AILevel.fromString(level)));
				return true;
			} catch (Exception e) {
